...
export default function adolescencia(data: any): any {
  ...
  // --- Salud visual ---
  let ojoDerecho: string | null = null;
  let ojoIzquierdo: string | null = null;
  ...
  let labelIMC = 'Normal';
  if (imc >= 25 && imc < 30) labelIMC = 'Sobrepeso';
  else if (imc >= 30) labelIMC = 'Obesidad';
  else if (imc < 18.5) labelIMC = 'Bajo peso';
  ...
  const exploracionCabeza =
    'NORMOCEFALO, PUPILAS ISOCORICAS NORMOREACTIVAS A LA LUZ, FOSAS NASALES PERMEABLES, CAVIDAD ORAL NORMAL';
  const exploracionCuello =
    'SIMETRICO, MOVIL, NO ADENOPATIAS, NO INGURGITACION YUGULAR';
  const exploracionTorax =
    'SIMETRICO, NO DEFORMIDADES, RSCS: RITMICOS, NO SOPLOS, BIEN TIMBRADOS; CSPS: VENTILADOS, NO ESTERTORES';
  const exploracionAbdomen =
    'BLANDO, DEPRESIBLE, NO MASAS PALPABLES, NO VICEROMEGALIAS, NO DOLOROSO A LA PALPACION, RSHS: NORMALES';
  const exploracionGenitourinario =
    'GENITALES EXTERNOS NORMALES, PUÑOPERCUSION NEGATIVA';
  const exploracionPelvis =
    'SIMETRICA, NO DEFORMIDADES, BUENA MOVILIDAD COXOFEMORAL';
  const exploracionDorso = 'SIMETRICAS, NO DEFORMIDADES, NO EDEMAS';
  const exploracionNeurologico =
    'GLASGOW 15/15, CONCIENTE, ORIENTADO EN TIEMPO Y ESPACIO, MOTRICIDAD Y SENSIBILIDAD GENERAL CONSERVADAS';
  const exploracionPiel =
    'HIDRATADA, ASPECTO Y COLORACION NORMAL, SIN LESIONES';
  ...
  const exploracionTactoRectal =
    'TACTO RECTAL NO REALIZADO (NO INDICADO PARA TAMIZAJE EN ESTE GRUPO ETARIO SIN SINTOMATOLOGÍA).';

  // ============================================================
  // 7. FUNCIONES DE MAPEO PARA RESOLUCION 4505
  // ============================================================

  const agudezaToCode = (valor: string | null): string => {
    if (!valor) return "3"; // Asumimos normal 20/20 si no hay dato
    return "3";
  };
  ...
  const mapHepatitisB = (resultado: string | null): { codigo: string; tieneResultado: boolean } => {
    if (!resultado) return { codigo: "0", tieneResultado: false };
    const r = resultado.trim().toLowerCase();
    if (r === "positivo" || r === "reactivo") return { codigo: "4", tieneResultado: true };
    if (r === "negativo" || r === "no reactivo") return { codigo: "5", tieneResultado: true };
  ...
  let tamizajeCuello: string;
  let resultadoCitologia: string;
  let calidadMuestra: string;
  let codigoIPS: string;
  let resultadoBiopsia: string;
  let fechaCitologia: string;
  let fechaColposcopia: string;
  let fechaBiopsia: string;
  let tratamientoAblativo: string;
  ...
  // ============================================================
  const procesarLaboratorio = (valor: number | null | undefined): { resultado: string; fecha: string } => {
