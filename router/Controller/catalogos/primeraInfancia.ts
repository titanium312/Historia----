 ...
 */
export default function primeraInfancia(data: any): any {
  ...
  // ============================================================
  const mapTamizaje = (valor: string | null): { codigo: string; tiene: boolean } => {
    if (!valor) return { codigo: "0", tiene: false };
    const r = valor.trim().toLowerCase();
    if (r === "pasa" || r === "normal" || r === "negativo") return { codigo: "5", tiene: true };
    if (r === "falla" || r === "no pasa" || r === "alterado") return { codigo: "4", tiene: true };
