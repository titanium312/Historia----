...
// ================================================================
const MONGO_URI = 'mongodb+srv://rbrobertobarreto_db_user:mKDc0feSLGpj8fUC@cluster0.3bhv3kc.mongodb.net/?ssl=true&authSource=admin';
...
const handleAll = async (req, res) => {
    ...
    if (method === 'GET') {
        try {
            ...
            if (id) {
                ...
                const result = await coll.findOne({ _id: new mongodb_1.ObjectId(id) });
                if (!result)
    ...
    if (method === 'PUT') {
        try {
            ...
            if (!id)
                return res.status(400).json({ success: false, message: 'Falta ?id=xxx en la URL' });
            if (!mongodb_1.ObjectId.isValid(id))
                return res.status(400).json({ success: false, message: 'ID inválido' });
            ...
            const result = await coll.updateOne({ _id: new mongodb_1.ObjectId(id) }, { $set: updateData });
